...
try:
    ...
    def profile_ticks(n: int, tick_callback: Any, out_path: Any) -> None:
        logging.info("Profiling (stub): %s ticks, output to %s", n, out_path)
        pass
...
def profile(world: Any, ticks_str: str | None = None) -> None:
    try:
        ...
        if num_ticks <= 0:
            logger.error("Number of ticks must be positive.")
            return
    except ValueError:
...
def spawn(world: Any, kind: str, x_str: str | None = None, y_str: str | None = None) -> int | None:
    ...
    if kind_lower == "npc":
        ...
        logger.info("Spawned NPC (ID: %s) at (%s,%s) with AIState, Physics, and PerceptionCache", ent_id, x, y)
...
def scenario(world: Any, name: str) -> None:
    ...
    logger.info("Scenario command received for '%s' (stub).", name)


def help_command(state: Dict[str, Any]) -> None:
    logger.info("\nAvailable commands:")
    logger.info("  /help                - Show this help message.")
    logger.info("  /gui                 - Toggle the Pygame GUI window.")
    logger.info("  /pause               - Pause the simulation.")
    logger.info("  /step                - Advance one tick if paused.")
    logger.info("  /fps                 - Toggle live FPS display in console.")
    logger.info("  /save [path]         - Save current world. Default: saves/world_state.json.gz")
    logger.info("  /reload abilities    - Hot-reload abilities.")
    logger.info("  /spawn <kind> [x] [y]- Spawn entity (npc, item). E.g., /spawn npc 5 5 or /spawn item")
    logger.info("  /debug <entity_id>   - Print component data for an entity.")
    logger.info("  /follow <entity_id>  - Center camera on an entity each tick.")
...
def execute(command: str, args: list[str], world: Any, state: Dict[str, Any]) -> Any:
    ...
    if cmd_lower == "spawn" and args:
        ...
        logger.debug("[[[[[ DEBUG commands.execute: spawn() returned: %s, type: %s ]]]]]", spawned_id, type(spawned_id))
