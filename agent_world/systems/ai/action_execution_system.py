        ...
        while True:
            ...
            # --- HANDLE GenerateAbilityAction ---
                ...
                try:
                    ...
                    logging.info("[Tick %s][ActionExec] %s", tick, log_msg)
                except Exception as e:
                    error_msg = (
                        f"Error during ability generation for agent {action.actor} "
                        f"(desc: '{action.description}'): {e}"
                    )
                    logging.error("[Tick %s][ActionExec ERROR] %s", tick, error_msg)
                finally:
            ...
            # --- HANDLE UseAbilityAction ---
                ...
                if ability_system_instance:
                    ...
                    logging.error(
                        "[Tick %s][ActionExec ERROR] AbilitySystem not found. Cannot execute UseAbilityAction for agent %s.",
                        tick,
                        action.actor,
                    )
